...
$(document).ready(function() {
    $('#content h2 .add').click(function() {
		popload('添加管理用户',800,380,'/dhdSystem/dhd_system/?s=Home/User/add/');
		addDiv($('#iframe_pop'));
		popclose();
	});
	$('#content #table .tr .edit').click(function(event) {
		...
		if (id=='' || isNaN(id)) {
			...
			popload('修改用户信息',800,420,'/dhdSystem/dhd_system/?s=Home/User/saveuser/id/'+id);
			addDiv($('#iframe_pop'));
			popclose();
		}
	});
	$('#content #table .tr .del').click(function(event) {
		...
		if (id=='' || isNaN(id)) {
			...
			$.ajax({
				url:'/dhdSystem/dhd_system/?s=Home/User/userdel/',
				dataType:'json',
				type:'POST',
				data:'post=ok&id='+id,
	...
	$('#dely').click(function(event) {
		...
		if (delid=='') {
			...
			$.ajax({
				url:'/dhdSystem/dhd_system/?s=Home/User/in_user_del/',
				dataType:'JSON',
				type:'POST',
				data:'delid='+delid,
				success: function(data) {
...
<div id="content">
    ...
    <h2>
    	<div class="h2_left">
        	<a href="/dhdSystem/dhd_system/index.php/home/user/user_index" class="whole">全部</a>
            <a href="javascript:;" class="add">新增</a>
            <a href="javascript:history.back();" class="Retreat">后退</a>
            <a href="javascript:history.go(1);" class="Advance">前进</a>
        </div>
        <div class="search">
        	<form action="/dhdSystem/dhd_system/index.php/home/user/user_index" method="get">
  	...
  	<div class="tableBox">
  		<table id="table" border="1" bordercolor="#CCCCCC" cellpadding="0" cellspacing="0">
	        ...
	        <?php if(is_array($volist)): $i = 0; $__LIST__ = $volist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="tr <?php if(($mod) == "1"): ?>tr2<?php endif; ?>">
	            ...
	            <td class="tc">
	            <?php if($vo["state"] == 1): ?><img src="/dhdSystem/dhd_system/Public/home/images/yes.png" border="0" title="正常" />
	            <?php else: ?>
	            <img src="/dhdSystem/dhd_system/Public/home/images/no.png" border="0" title="锁定" /><?php endif; ?>
	            </td>
